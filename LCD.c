...
void LCD_Clock_Init(void)
{
	...
	if ((PWR->CR1 & PWR_CR1_DBP) == 0)
	{
		...
		while ((PWR->CR1 & PWR_CR1_DBP) == 0)
			; // Wait for Backup domain Writ0 e protection disable
	}
...
void LCD_Configure(void)
{
	...
	while ((LCD->SR & LCD_SR_ENS) == 0)
		; // ENS is set by hardware automatically

	// Wait until the LCD Booster is ready
	while ((LCD->SR & LCD_SR_RDY) == 0)
		; // Loop until step-up converter is ready to provide the correct voltage.
